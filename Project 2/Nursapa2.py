...
class Form( QDialog):
    ...
    def buttonLoadFilePressed(self):
        ...
        def loadFile(): 
            def CreateTables():
                ...
                cur.execute("create table Sales(TrxId int not null, DoY int, StoreId int, SoupId int, PromoId int, Sales number, unique(TrxId));")
                cur.execute("Drop Table if Exists Days")
                cur.execute("create table Days(DoY int not null, DoW text, Holiday int, Weather text, unique(DoY));")
                cur.execute("Drop Table if Exists Stores")
                cur.execute("create table Stores(StoreId int not null, Location text, Elevation text, Size text, MgrId int, unique(StoreId));")
                cur.execute("Drop Table if Exists Managers")
                cur.execute("create table Managers(MgrId int not null, MgrName text, Grade text, Years int, unique(MgrId));")
                cur.execute("Drop Table if Exists Soups")
                cur.execute("create table Soups(SoupId int not null, Type text, Vendor text, Mode text, Style text, unique(SoupId));")
                cur.execute("Drop Table if Exists Promotions")
                cur.execute("create table Promotions(PromoId int not null, Medium text, Target text, Interval text, unique(PromoId));")
            CreateTables()
            def InsertSales(record):
                        ...
                        rowa = [TrxId, DoY, StoreId, SoupId, PromoId, Sales]
                        cur.execute("INSERT or ignore INTO Sales VALUES(?,?,?,?,?,?)", rowa)
            ...
            def InsertDays(record):
                        ...
                        rowb = [DoY, DoW, Holiday, Weather]
                        cur.execute("INSERT or ignore INTO Days VALUES(?,?,?,?)", rowb)
            ...
            def InsertStores(record):
                        ...
                        Size = str(record[8])
                        MgrId = int(record[9])
                        rowc = [StoreId, Location, Elevation, Size, MgrId]
                        cur.execute("INSERT or ignore INTO Stores VALUES(?,?,?,?,?)", rowc)
            ...
            def InsertManagers(record):
                        ...
                        Grade = str(record[11])
                        Years = int(record[12])
                        rowd = [MgrId, MgrName, Grade, Years]
                        cur.execute("INSERT or ignore INTO Managers VALUES(?,?,?,?)", rowd)
                                    
                    
            def InsertSoups(record):
                        SoupId = int(record[13])
                        Type = str(record[14])
                        Vender = str(record[15])
                        Mode = str(record[16])
                        Style = str(record[17])
                        rowe = [SoupId, Type, Vender, Mode, Style]
                        cur.execute("INSERT or ignore INTO Soups VALUES(?,?,?,?,?)", rowe)
            ...
            def InsertPromotions(record):
                        ...
                        rowf = [PromoId, Medium, Target, Interval]
                        cur.execute("INSERT or ignore INTO Promotions VALUES(?,?,?,?)", rowf)  
            ...
            try:
                ...
                linecount = 0
                line = f.readline()
                while line != "" and linecount < 70000:
    ...
    def buttonTableRowsCountPressed(self):
        conn = sqlite3.connect('Nursapa.db')
        cur = conn.cursor()
        tableName = str(self.lineeditTableRowsCount.text())
